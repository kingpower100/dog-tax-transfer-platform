      ...
      <div className="flex gap-2">
        {citizens.map(c => (
          <button
            ...
            className={`px-3 py-1 rounded-full font-bold transition whitespace-nowrap ${currentCitizen?.user?.id === c.id || currentCitizen?.id === c.id ? 'bg-blue-600' : 'bg-slate-800 hover:bg-slate-700'}`}
          >
            {c.name}
          </button>
        ))}
